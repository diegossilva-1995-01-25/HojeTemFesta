...
public class Classe0 {
	...
	// A mensagem será exibida em um pop-up.
	public static void exibirEmTelaAniversarianteHoje() {
		...
		if(cont > 0) {
			JOptionPane.showMessageDialog(null, "Que as mais seletas bênçãos possam vos\n"
					+ "alcançar e que Jesus vos multiplique vossos\n"
					+ "anos de vida e vos dê a Vida Eterna!!!\n;-D");
		} else {
	...
	public static void exibirEmTelaAniversarianteDiasPassados(int nDiasAtras) {
		...
		while(nDiasAtras > 0) {
			...
			while(cont < listaAniversariantes.size()) {
				...
				JOptionPane.showMessageDialog(null, "Parabéns,\n" + m.getCargo() + " "
						+ m.getNome() + " \"" + m.getApelido() + "\" " + m.getSobrenomes()
						+ "\npor seu aniversário em " + formato.format(LocalDate.now().minusDays(nDiasAtras)));
